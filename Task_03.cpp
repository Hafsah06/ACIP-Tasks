...
const double MAX_WEIGHT_DIFFERENCE = 2.5;
...
int main()
{
	...
	for (int i = 0; i < classSize; ++i)
	{
		...
		if (abs(weightDifference[i] > MAX_WEIGHT_DIFFERENCE))
		{
			cout << "Warning: " << names[i] << " 's weight has changed by more than 2.5 kilograms.";

			if (weightDifference[i] > 0)
			{
				cout << "This is a rise." << endl;
			}
			else
			{
				cout << "This is a fall." << endl;
			}
		}
	}
